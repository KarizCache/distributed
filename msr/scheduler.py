...
class TaskState:
    ...
    .. attribute:: timing_info: TimingInfo or dict version

       The execution duration info of this task.

    .. attribute:: critical_task: TaskState
    
       The dependency of this task with the longest critical path time.

    .. attribute:: critical_path_time: float

       The sum of execution durations on this task's critical path.
       Used when recursively calculating critical_task.
    """
    ...
    __slots__ = (
        ...
        "critical_task",
        "critical_path_time",
        # Color in a chain decomposition of the task graph (RF)
        "color",
        # Color of the child to whom we are pushing the node's output (RF)
        "child_color"
    )
    ...
    def __init__(self, key, run_spec):
        ...
        self.group = None
        self.timing_info = None  # Populated upon return from worker thread.
...
class Scheduler(ServerNode):
    ...
    * **task_duration:** ``{key-prefix: time}``
    ...
    def generate_graph_chains(self):
        """
    ...
    def update_graph(
        ...
        # RF: Coloring nodes according to chains
        self.generate_graph_chains()
    ...
    def send_task_to_worker(self, worker, key):
        ...
        try:
            ...
            if ts.color:
                msg["color"] = ts.color
                msg["child_color"] = ts.child_color
    ...
    async def get_timing_info(self, comm=None, key=None, serializers=None):
        # Get the timing info for the task (and all dependencies on critical path).
        drilldown = []
        ts = self.tasks.get(key)
        while ts is not None:
            drilldown.append(ts.timing_info)
            ts = ts.critical_task
        drilldown = reversed(drilldown)
        return [
            ti.to_dict()
        ...
        for dts in deps:
            ...
            if ts in s:
                ...
                if not s:  # new task ready to run
                    # Figure out the critical task.
                    dependencies = dts.dependencies
                    longest_dependency = None
                    for dep in dependencies:
                        if dep.critical_path_time is None:
                            # logger.warn("Critical path time not being tracked.")
                            # break
                            logger.warn("Skipping critical path for {}".format(dep.key))
                            continue
                        if longest_dependency is None:
                            longest_dependency = dep
                        elif dep.critical_path_time > longest_dependency.critical_path_time:
                            longest_dependency = dep
                    dts.critical_task = longest_dependency
    ...
    def transition_processing_memory(
        ...
        **kwargs,
        ...
        try:
            ...
            if ser_timing_info is not None:
                ts.timing_info = pickle.loads(ser_timing_info)
                ts.critical_path_time = 0
                if ts.critical_task is not None:  # nonleaf tasks
                    ts.critical_path_time += ts.critical_task.critical_path_time
                ts.critical_path_time += ts.timing_info.duration
